...
static u8 g_proximity_en = 0;
	...
	{
		#include "20151228_ASUS_Z170_ES12_0D.h"
	};
#endif
...
bool himax_has_gesture = false;
bool himax_kernel_suspend = false;
#endif
...
/*static int i2c_himax_read_command(struct i2c_client *client, uint8_t length, uint8_t *data, uint8_t *readlength, uint8_t toRetry)
{
	int retry;
	struct i2c_msg msg[] = {
		{
...
static int i_update_FW(void)
{
	...
	uint8_t vendor_fw_ver = private_ts->vendor_fw_ver;
	uint8_t vendor_config_ver = private_ts->vendor_config_ver;

	I("private_ts->vendor_fw_ver-x: %x, i_CTPM_FW[FW_VER_MAJ_FLASH_ADDR-x]: %x \n", vendor_fw_ver, i_CTPM_FW[FW_VER_MAJ_FLASH_ADDR]);
	I("private_ts->vendor_config_ver-x: %x, i_CTPM_FW[FW_CFG_VER_FLASH_ADDR-x]: %x \n", vendor_config_ver, i_CTPM_FW[FW_CFG_VER_FLASH_ADDR]);

	if (( vendor_fw_ver < i_CTPM_FW[FW_VER_MAJ_FLASH_ADDR] )
		|| ( vendor_config_ver < i_CTPM_FW[FW_CFG_VER_FLASH_ADDR] )
		|| ( himax_calculateChecksum(false) == 0 ) )
		{
flash_retry:
			i++;
			I("The vendor_ver is small than i_CTPM_FW start to update fw.\n");
...
static void himax_read_TP_info(struct i2c_client *client)
{
	...
	void himax_write_usb_status(void) 
	{
		int status = ts->usb_status;
		I("cable_status=%d \n", status);
        if (ts->cable_config) {
            if (status == 0x02) {   /*no AC */
                I("USB OUT\n");
                ts->cable_config[1] = 0x00;
            } else if ((status == 0x00) || (status == 0x01)) {  /*AC plug in*/
                I("USB IN\n");
...
#if defined(HX_USB_DETECT)
		I("Detect USB and Change cable config - Start");


		wake_lock(&ts->ts_wake_lock);
		mutex_lock(&ts->ts_mutex);
		
		himax_write_usb_status();
		
		mutex_unlock(&ts->ts_mutex);
		wake_unlock(&ts->ts_wake_lock);
#endif

		I("Detect USB and Change cable config - End");
	}
#endif
...
static int himax_parse_wake_event(struct himax_ts_data *ts)
{
	...
	if((buf[0]==0x57)&&(buf[1]==0x61)&&(buf[2]==0x6B)&&(buf[3]==0x65))/*W,A,K,E*/
		{
			I("%s: WAKE UP system!\n", __func__);
			return EV_GESTURE_U;//Yes, wake up system
		}
	else if((buf[0]==0x47)&&(buf[1]==0x45)&&(buf[2]==0x53)&&(buf[3]==0x55))/*G,E,S,U*/
		{
			I("%s: Gesture U Event!\n", __func__);
			return EV_GESTURE_U;//Send gesture U event
		}
	else if((buf[0]==0x47)&&(buf[1]==0x45)&&(buf[2]==0x53)&&(buf[3]==0x56))/*G,E,S,V*/
		{
			I("%s: Gesture V Event!\n", __func__);
			return EV_GESTURE_V;//Send gesture V event
		}
	else if((buf[0]==0x47)&&(buf[1]==0x45)&&(buf[2]==0x53)&&(buf[3]==0x57))/*G,E,S,W*/
		{
			I("%s: Gesture W Event!\n", __func__);
			return EV_GESTURE_W;//Send gesture W event
		}
	else if((buf[0]==0x47)&&(buf[1]==0x45)&&(buf[2]==0x53)&&(buf[3]==0x45))/*G,E,S,E*/
		{
			I("%s: Gesture E Event!\n", __func__);
			return EV_GESTURE_E;//Send gesture E event
		}
	else if((buf[0]==0x47)&&(buf[1]==0x45)&&(buf[2]==0x53)&&(buf[3]==0x43))/*G,E,S,C*/
		{
			I("%s: Gesture C Event!\n", __func__);
			return EV_GESTURE_C;//Send gesture C event
		}
	else if((buf[0]==0x47)&&(buf[1]==0x45)&&(buf[2]==0x53)&&(buf[3]==0x53))/*G,E,S,S*/
		{
			I("%s: Gesture S Event!\n", __func__);
			return EV_GESTURE_S;//Send gesture S event
		}
	else if((buf[0]==0x47)&&(buf[1]==0x45)&&(buf[2]==0x53)&&(buf[3]==0x5A))/*G,E,S,Z*/
		{
			I("%s: Gesture Z Event!\n", __func__);
			return EV_GESTURE_Z;//Send gesture Z event
		}
	else
		{
			...
			if(vk_press == 0x00)
				{
					...
					for (loop_i = 0; loop_i < ts->nFinger_support; loop_i++) {
						if (((finger_pressed >> loop_i) & 1) == 1) {
							...
							if((!atomic_read(&ts->suspend_mode))&&(x==0x0064)&&(y==0x0064)&&(w==0x50))
							{
...
#ifdef HX_SMART_WAKEUP
static bool himax_read_gesture(struct himax_ts_data *ts)
{
	...
	ret_event = himax_parse_wake_event(ts);
	switch (ret_event) {
		case EV_GESTURE_U:
			KEY_EVENT = KEY_POWER;
		break;
...
#ifdef HX_SMART_WAKEUP
	if (atomic_read(&ts->suspend_mode)&&(!FAKE_POWER_KEY_SEND)&&(ts->gesture_all)) {
		if(himax_kernel_suspend) {
			himax_has_gesture = true;
			return IRQ_HANDLED;
		} else {
			if(himax_read_gesture(ptr)) {
				return IRQ_HANDLED;
			}
		}
	}
...
static int himax_power_on(struct himax_ts_data *data, bool on)
{
	int rc;
	...
	rc = reg_set_optimum_mode_check(data->vcc_ana, HX_ACTIVE_LOAD_UA);
	if (rc < 0) {
	...
	if (data->pdata->i2c_pull_up) {
		rc = reg_set_optimum_mode_check(data->vcc_i2c, HX_I2C_LOAD_UA);
		if (rc < 0) {
...
static int himax_regulator_configure(struct himax_ts_data *data, bool on)
{
	int rc;
	...
	if (regulator_count_voltages(data->vcc_ana) > 0) {
		rc = regulator_set_voltage(data->vcc_ana, HX_VTG_MIN_UV,
							HX_VTG_MAX_UV);
		if (rc) {
...
error_get_vtg_vcc_dig:
	if (regulator_count_voltages(data->vcc_ana) > 0)
		regulator_set_voltage(data->vcc_ana, 0, HX_VTG_MAX_UV);
...
static int himax_gpio_power_config(struct himax_ts_data *data)
{
	int error;
	
	error = himax_regulator_configure(data, true);
	if (error) {
	...
	if (gpio_is_valid(data->pdata->gpio_reset)) {
		...
		error = gpio_request(data->pdata->gpio_reset, "hmx_reset_gpio");
		if (error) {
	...
	error = himax_power_on(data, true);
	if (error) {
	...
	if (gpio_is_valid(data->pdata->gpio_irq)) {
		...
		error = gpio_request(data->pdata->gpio_irq, "hmx_gpio_irq");
		if (error) {
			E("unable to request gpio [%d]\n",
						data->pdata->gpio_irq);
			goto err_power_on;
		...
		data->irq = data->client->irq = gpio_to_irq(data->pdata->gpio_irq);
	} else {
...
err_power_on:
		himax_power_on(data, false);
err_gpio_reset_req:
	if (gpio_is_valid(data->pdata->gpio_reset))
		gpio_free(data->pdata->gpio_reset);
err_regulator_on:
		himax_regulator_configure(data, false);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (value) {
		ret = request_threaded_irq(ts->client->irq, NULL, himax_ts_thread,IRQF_TRIGGER_FALLING | IRQF_ONESHOT | IRQF_NO_SUSPEND, ts->client->name, ts);
		if (ret == 0) {
			ts->irq_enabled = 1;
			irq_enable_count = 1;
		}
...
static ssize_t himax_diag_dump(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
	...
	I("[Himax]2T3R=0x%x, command=0x%x\n",Is_2T3R, buf[0]);
...
static int himax_chip_self_test(void)
{
    ...
    i2c_himax_read(private_ts->client, 0x5A, valuebuf, 1, DEFAULT_RETRY_CNT);
    msleep(10);
    ...
    valuebuf[0] = valuebuf[0] & 0xF5; // close re-calibration, idle mode
    ...
    i2c_himax_write(private_ts->client, 0x4B, valuebuf, 1, DEFAULT_RETRY_CNT);
    msleep(10);
    cmdbuf[0] = 0x00;
    i2c_himax_write(private_ts->client, 0x8C,&cmdbuf[0], 1, DEFAULT_RETRY_CNT);
    msleep(10);
...
static ssize_t himax_GESTURE_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct himax_ts_data *ts = private_ts;
	size_t count = 0;
	count = sprintf(buf, "gesture all:%d, w:%d, s:%d, e:%d, c:%d, z:%d, v:%d\n", 
			ts->gesture_all, ts->gesture_w, ts->gesture_s, ts->gesture_e, ts->gesture_c, ts->gesture_z, ts->gesture_v);
	return count;
}

static ssize_t himax_GESTURE_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct himax_ts_data *ts = private_ts;

	I("Input gesture_all_w_s_e_c_z_v = %s", buf);

	//[0][1][2][3][4][5][6] mapping to [all][w][s][e][c][z][v]
	if (buf[0] == '0')
		ts->gesture_all = 0;
	else if (buf[0] == '1')
		ts->gesture_all = 1;
	...
	I("gesture all:%d, w:%d, s:%d, e:%d, c:%d, z:%d, v:%d\n", ts->gesture_all, ts->gesture_w, ts->gesture_s, ts->gesture_e, ts->gesture_c, ts->gesture_z, ts->gesture_v);

	return count;
}

static DEVICE_ATTR(GESTURE, (S_IWUSR|S_IRUGO|S_IWUGO),
	himax_GESTURE_show, himax_GESTURE_store);
#endif
static int himax_touch_sysfs_init(void)
{
	...
	if (ret)
	{
		...
		ret = sysfs_create_file(android_touch_kobj, &dev_attr_GESTURE.attr);
		if (ret)
		{
			E("sysfs_create_file dev_attr_GESTURE failed\n");
			return ret;
		}
	#endif
...
static void himax_touch_sysfs_deinit(void)
{
	...
	sysfs_remove_file(android_touch_kobj, &dev_attr_GESTURE.attr);
	#endif
		...
		if(HX_INT_IS_EDGE)
			{
				...
				ret = request_threaded_irq(client->irq, NULL, himax_ts_thread,IRQF_TRIGGER_FALLING | IRQF_ONESHOT | IRQF_NO_SUSPEND, client->name, ts);
			}
		else
			{
...
static int check_lcm_id(void)
{
	...
	I("LCM ID :%d\n", lcm_id);
	if (lcm_id != KD && lcm_id != HH_HIMAX ){
		I("LCM ID is not KD or HH\n");
		return error;
	}
	return ret;
}
...
int setHimaxTouchACMode(int usb_states)
{
	ts->usb_status = usb_states;
	I("ts->usb_status = %d, usb_state = %d \n", __func__, ts->usb_status, usb_states);
	//only write usb mode to MCU in wake status and at end of resume
	if(!ts->suspended) {
		queue_work(ts->himax_usb_wq, &ts->usb_detect_work);
	}
	return 0;
}

static int himax_cable_status_notify(struct notifier_block *self,
			       unsigned long action, void *dev)
{
	switch (action) {
	case POWER_SUPPLY_CHARGER_TYPE_USB_SDP:
		I("POWER_SUPPLY_CHARGER_TYPE_USB_SDP !!!\n");
		setHimaxTouchACMode(0);
		break;

	case POWER_SUPPLY_CHARGER_TYPE_USB_CDP:
		I("POWER_SUPPLY_CHARGER_TYPE_USB_CDP !!!\n");
		setHimaxTouchACMode(1);
		break;

	case POWER_SUPPLY_CHARGER_TYPE_USB_DCP:
		I("POWER_SUPPLY_CHARGER_TYPE_USB_DCP !!!\n");
		setHimaxTouchACMode(1);
		break;

	case POWER_SUPPLY_CHARGER_TYPE_ACA_DOCK:
		I("POWER_SUPPLY_CHARGER_TYPE_ACA_DOCK !!!\n");
		setHimaxTouchACMode(1);
		break;

	case POWER_SUPPLY_CHARGER_TYPE_SE1:
		I("POWER_SUPPLY_CHARGER_TYPE_SE1 !!!\n");
		setHimaxTouchACMode(1);
		break;
#if 0
	...
	case POWER_SUPPLY_CHARGER_TYPE_NONE:
		I("POWER_SUPPLY_CHARGER_TYPE_NONE !!!\n");
		setHimaxTouchACMode(2);
		break;
...
static struct notifier_block himax_cable_status_notifier = {
	.notifier_call = himax_cable_status_notify,
};

#ifdef HX_SMART_WAKEUP
int himax_gesture_switch(void)
{
	uint8_t gesture_all[2]= {0xF4, 0x00};
	int ret = 0;
	uint8_t gesture_register[1];

	ts->SMWP_enable ? (gesture_all[1] |= GESTURE_DOUBLE_CLICK) : (gesture_all[1] |= ~GESTURE_DOUBLE_CLICK);
	ts->gesture_v ? (gesture_all[1] |= GESTURE_V) : (gesture_all[1] |= ~GESTURE_V);
	ts->gesture_w ? (gesture_all[1] |= GESTURE_W) : (gesture_all[1] |= ~GESTURE_W);
	ts->gesture_e ? (gesture_all[1] |= GESTURE_E) : (gesture_all[1] |= ~GESTURE_E);
	ts->gesture_c ? (gesture_all[1] |= GESTURE_C) : (gesture_all[1] |= ~GESTURE_C);
	ts->gesture_s ? (gesture_all[1] |= GESTURE_S) : (gesture_all[1] |= ~GESTURE_S);
	ts->gesture_z ? (gesture_all[1] |= GESTURE_Z) : (gesture_all[1] |= ~GESTURE_Z);

	ret = i2c_himax_master_write(ts->client, gesture_all, 2, HIMAX_I2C_RETRY_TIMES);

	i2c_himax_read(ts->client, gesture_all[0], gesture_register, 1, HIMAX_I2C_RETRY_TIMES);
	I("Gesture switch register: 0x%02x \n", gesture_register[0]);
...
#ifdef HX_SMART_WAKEUP
static int himax852xes_kernel_resume(struct i2c_client *client)
{
	if(himax_has_gesture) {
		himax_has_gesture = false;
		struct himax_ts_data *ts = i2c_get_clientdata(client);
		himax_read_gesture(ts);
	}
	himax_kernel_suspend = false;
	return 0;
}

static int himax852xes_kernel_suspend(struct i2c_client *client, pm_message_t mesg)
{
	himax_kernel_suspend = true;
	return 0;
}
#endif
...
static int himax852xes_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	I("system entry_mode = %d \n",entry_mode);

	if(entry_mode != 1 && entry_mode != 4){
            I("Not in MOS and COS, don't need touch driver. \n");
            return ret;
	}
	// In COS mode
	if(entry_mode == 4){
...
#else
		ts->pdata = pdata;
		himax_gpio_power_config(ts);
#endif

	//Make sure correct power on sequence, need do HW RESET
	gpio_direction_output(ts->rst_gpio, 0);
	msleep(30);
	gpio_direction_output(ts->rst_gpio, 1);
	msleep(30);
	...
	if (ret) {
		...
		goto err_input_register_device_failed;
          ...
          __set_bit(KEY_E, ts->input_dev->keybit);
          __set_bit(KEY_W, ts->input_dev->keybit);
          __set_bit(KEY_S, ts->input_dev->keybit);
          __set_bit(KEY_V, ts->input_dev->keybit);
          __set_bit(KEY_Z, ts->input_dev->keybit);
          __set_bit(KEY_C, ts->input_dev->keybit);
#endif
	...
	ts->himax_usb_wq = create_singlethread_workqueue("HMX_USB_wq");
	if (!ts->himax_usb_wq) {
		E(" allocate HMX_USB_wq failed\n");
		err = -ENOMEM;
		goto err_get_intr_bit_failed;
	}
	INIT_DELAYED_WORK(&ts->usb_detect_work, himax_cable_status);
...
#if defined(HX_USB_DETECT)
	I("Init charger workqueue\n");

	if (ts->cable_config)
		cable_status_register_client(&himax_cable_status_notifier);
#endif
#ifdef HX_DOT_VIEW
	...
	if(cos_flager){
		uint8_t buf[1] = {0};
		I("In charger mode, Himax IC will sleep\n");
...
#endif
#if defined(HX_USB_DETECT)
	cable_status_unregister_client(&himax_cable_status_notifier);
#endif
	if (!ts->use_irq)
		hrtimer_cancel(&ts->timer);
...
static int himax852xes_suspend(struct device *dev)
{
	...
	if(ts->gesture_all)
	{
		...
		ret = himax_gesture_switch();
		if (ret < 0)
		{
			E("[himax] %s: I2C access failed form gesture");
		}
		...
		I("Disable GESTURE and DOUBLE CLICK \n");
	}
#endif
...
//set USB mode at the end of resume
#if defined(HX_USB_DETECT)
	himax_write_usb_status();
#endif
	return 0;
...
#ifdef HX_SMART_WAKEUP
	.suspend	= himax852xes_kernel_suspend,
	.resume		= himax852xes_kernel_resume,
#endif
